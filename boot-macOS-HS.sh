...
#MY_OPTIONS="+vmx,+nx,+ssse3,+sse4.1,+sse4.2"
MY_OPTIONS="+aes,+xsave,+avx,+xsaveopt,avx2,+smep,vmx,+nx,+ssse3,+sse4.1,+sse4.2"
...
qemu-system-x86_64 -enable-kvm -m 4096 -cpu Penryn,kvm=on,vendor=GenuineIntel,+invtsc,vmware-cpuid-freq=on,$MY_OPTIONS\
    ...
    -drive if=pflash,format=raw,file=OVMF_VARS-pure-efi-1600x900.fd \
    -smbios type=2 \
    ...
    -drive id=MacDVD,if=none,snapshot=on,media=cdrom,file=./'macOS_HighSierra_10.13.iso' \
